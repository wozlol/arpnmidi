/*
  ...
  - USB_HUB_RELEASE_COMMAND_TIMEOUT_MS is 0 by default, so the hub stays in reset until the main
    brain requests it. Set it nonzero only when deliberately testing without the command link.
...
constexpr uint32_t USB_HUB_RELEASE_COMMAND_TIMEOUT_MS = 0;
